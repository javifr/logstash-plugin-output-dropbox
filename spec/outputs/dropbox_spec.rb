...
describe LogStash::Outputs::Dropbox do
  ...
  #     s3 = LogStash::Outputs::Dropbox.new(config.merge({ "region" => 'sa-east-1' }))
  ...
  #     s3 = LogStash::Outputs::Dropbox.new(minimal_settings.merge({ "temporary_directory" => "/tmp/logstash" }))
  ...
  #     expect_any_instance_of(AWS::Dropbox::ObjectCollection).to receive(:[]).with("#{prefix}#{File.basename(fake_data)}") { fake_bucket }
  ...
  #     expect_any_instance_of(AWS::Dropbox::ObjectCollection).to receive(:[]).with(File.basename(fake_data)) { fake_bucket }
  ...
  #     s3 = LogStash::Outputs::Dropbox.new(minimal_settings.merge({ "size_file" => 200 }))
  ...
  #   let(:s3) { LogStash::Outputs::Dropbox.new(minimal_settings.merge({ "size_file" => 1024 })) }
  ...
  #       s3 = LogStash::Outputs::Dropbox.new(minimal_settings.merge(config))
